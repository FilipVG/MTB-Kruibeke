...
/**
 * Update-mail voor wie al ingeschreven is: de rit is gewijzigd na de
 * uitnodiging. Toont de bijgewerkte gegevens. Titel: "Update <ritnaam>".
 * `changes` is optioneel — een lijstje met wat er gewijzigd is.
 */
export function buildRideUpdateEmail(
  ride: RideInfo,
  siteUrl: string,
  changes: string[] = [],
): { subject: string; html: string } {
  const rideUrl = `${siteUrl}/kalender/${ride.id}`;
  ...
  const changesBlock = changes.length > 0
    ? `<table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 8px;">
         <tr><td style="padding:14px 20px;background:#fef3c7;border:1px solid #fcd34d;border-radius:8px;">
           <p style="margin:0 0 6px;font-size:13px;font-weight:700;color:#92400e;text-transform:uppercase;letter-spacing:0.05em;">Wat is er gewijzigd?</p>
           <ul style="margin:0;padding-left:18px;font-size:14px;color:#1a1a1a;line-height:1.7;">
...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:'Segoe UI',Arial,sans-serif;">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:32px 32px 0;">
            <h2 style="margin:0 0 8px;font-size:20px;font-weight:700;color:#1a1a1a;">Wijziging aan een rit</h2>
            <p style="margin:0;font-size:15px;color:#555;line-height:1.6;">
              Je bent ingeschreven voor deze rit. De gegevens zijn bijgewerkt — hieronder de nieuwe info.
            </p>
          </td>
        </tr>

        <!-- Wijzigingen (optioneel) -->
        ${changesBlock ? `<tr><td style="padding:20px 32px 0;">${changesBlock}</td></tr>` : ''}
        ...
        <tr>
          <td style="padding:0 32px 28px;" align="center">
            <a href="${rideUrl}" style="display:inline-block;background:#b91c1c;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;padding:13px 28px;border-radius:8px;">Bekijk de rit</a>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#1a1a1a;padding:20px 32px;">
            <p style="margin:0;font-size:12px;color:#888;text-align:center;">MTB Kruibeke · Mountainbike in het Waasland</p>
          </td>
        </tr>
  ...
  return { subject: `Update ${ride.title}`, html };
}
