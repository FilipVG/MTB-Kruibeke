...
export function ProfileForm({ profile }: { profile: Profile }) {
  ...
  async function handleCropDone(blob: Blob) {
    ...
    if (error) { setMessage(`Upload mislukt: ${error.message}`); setSaving(false); return; }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase
      .from('profiles')
      .update({ ...form, birthdate: form.birthdate || null })
      .eq('id', profile.id);
    if (error) {
  ...
  return (
    <>
      {cropSrc && (
        <AvatarCropper imageSrc={cropSrc} onCrop={handleCropDone} onCancel={() => setCropSrc(null)} />
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="card p-6">
          ...
          <label className="flex items-center gap-3 cursor-pointer mt-3">
            <input
              type="checkbox"
              checked={form.review_reminders}
              onChange={e => setForm({ ...form, review_reminders: e.target.checked })}
              className="rounded border-ink-700 bg-ink-900 text-brand-700 focus:ring-brand-500 h-4 w-4"
            />
            <div>
          ...
          <label className="flex items-center gap-3 cursor-pointer mt-3">
            <input
              ...
              onChange={e => setForm({ ...form, wants_newsletter: e.target.checked })}
              className="rounded border-ink-700 bg-ink-900 text-brand-700 focus:ring-brand-500 h-4 w-4"
            />
            <div>
              <p className="text-sm text-ink-200">Off-Road Update ontvangen</p>
              <p className="text-xs text-ink-500">Ontvang de clubnieuwsbrief met agenda en aankondigingen.</p>
            </div>
          </label>
        </div>
